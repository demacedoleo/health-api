...
var (
	Statements = statements{
		...
		Selects: selects{
			...
			Meetings:        "select * from health.scheduler_meetings where start_time >= ? and end_time <= ?;",
			Staffs:          "select cs.id, cs.company_id, cs.document, cs.charge_type, cs.job_position, cs.status, p.document, p.person_name, p.last_name, p.birthday, p.gender from company_staffs as cs join person as p on cs.document = p.document where cs.company_id = ?;",
		},
	}
)
